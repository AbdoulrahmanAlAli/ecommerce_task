"use client";

import { UserShop } from "@/utils/types";
import axios from "axios";
import React, { useState, useEffect } from "react";
import { toast } from "react-toastify";

type ShoppingCartProps = {
  payload: {
    id: number;
    isAdmin: boolean;
    username: string;
    iat: number;
    exp: number;
  };
};

const DOMAIN = "http://localhost:3000";

const ShoppingCart = ({ payload }: ShoppingCartProps) => {
  const [userData, setUserData] = useState<UserShop | null>(null);
  const [total, setTotal] = useState<number>(0);
  const [loading, setLoading] = useState<boolean>(true);

  const data = payload.id;

  useEffect(() => {
    const fetchUser = async () => {
      if (data) {
        try {
          const response = await axios.get(
            `${DOMAIN}/api/users/profile/${data}`
          );
          setUserData(response.data);
          setLoading(false);
        } catch (error) {
          console.error("Error fetching user data:", error);
          setLoading(false);
        }
      }
    };
    fetchUser();
  }, [data]);

  useEffect(() => {
    if (userData?.products) {
      const totalAmount = userData.products.reduce(
        (acc, product) => acc + product.price,
        0
      );
      setTotal(totalAmount);
    }
  }, [userData]);

  if (loading) {
    return (
      <section className="w-full h-[90vh] p-5 flex items-center justify-center">
        <div role="status">
          <svg
            aria-hidden="true"
            className="w-16 h-16 text-gray-200 animate-spin dark:text-gray-600 fill-green-600"
            viewBox="0 0 100 101"
            fill="none"
            xmlns="http://www.w3.org/2000/svg">
            <path
              d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
              fill="currentColor"
            />
            <path
              d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
              fill="currentFill"
            />
          </svg>
        </div>
      </section>
    );
  }

  const handleRemoveProduct = async (productId: number) => {
    try {
      const response = await axios.put(`${DOMAIN}/api/users/addcart`, {
        id: productId,
      });
      if (response.status === 200) {
        const message = response.data.message;
        if (message === "Product added to cart") {
          toast.success("Product added to cart!");
        } else if (message === "Product removed from cart") {
          toast.warn("Product removed from cart!");
        }
      }
    } catch (error: any) {
      toast.error(error.response?.data?.message);
    }
  };

  return (
    <div className="max-w-3xl mx-auto p-6">
      <h2 className="text-3xl font-bold text-center mb-6">Shopping Cart</h2>
      {userData?.products && userData.products.length > 0 ? (
        <div>
          <ul className="space-y-4">
            {userData.products.map((product) => (
              <li
                key={product.id}
                className="flex justify-between items-center p-4 bg-white shadow-md rounded-md border">
                <div>
                  <h3 className="font-semibold">{product.title}</h3>
                  <p className="text-gray-500">{product.description}</p>
                </div>
                <div className="flex items-center space-x-4">
                  <span className="font-semibold text-green-600">
                    ${product.price.toFixed(2)}
                  </span>
                  <button
                    onClick={() => handleRemoveProduct(product.id)}
                    className="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700">
                    Remove
                  </button>
                </div>
              </li>
            ))}
          </ul>
          <div className="mt-6 text-lg font-semibold">
            <span className="text-xl font-bold">Total: </span>$
            {total.toFixed(2)}
          </div>
          <div className="mt-4">
            <button className="w-full bg-blue-600 text-white py-2 rounded-md hover:bg-blue-700">
              Buy Now
            </button>
          </div>
        </div>
      ) : (
        <p className="text-center text-gray-500">No products in your cart</p>
      )}
    </div>
  );
};

export default ShoppingCart;
